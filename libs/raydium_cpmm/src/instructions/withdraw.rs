  ...
  pub fn instruction(&self) -> solana_instruction::Instruction {
    let accounts = Withdraw {
          ...
          let args = WithdrawInstructionArgs {
                                                              lp_token_amount: self.lp_token_amount.clone().expect("lp_token_amount is not set"),
                                                                  minimum_token0_amount: self.minimum_token0_amount.clone().expect("minimum_token0_amount is not set"),
                                                                  minimum_token1_amount: self.minimum_token1_amount.clone().expect("minimum_token1_amount is not set"),
                                    };
...
#[inline(always)]
    pub fn pool_state(&mut self, pool_state: &'b solana_account_info::AccountInfo<'a>) -> &mut Self {
      ...
      /// Owner lp token account
#[inline(always)]
    pub fn owner_lp_token(&mut self, owner_lp_token: &'b solana_account_info::AccountInfo<'a>) -> &mut Self {
                        self.instruction.owner_lp_token = Some(owner_lp_token);
                    self
    }
      /// The token account for receive token_0,
#[inline(always)]
    pub fn token0_account(&mut self, token0_account: &'b solana_account_info::AccountInfo<'a>) -> &mut Self {
                        self.instruction.token0_account = Some(token0_account);
                    self
    }
      /// The token account for receive token_1
#[inline(always)]
    pub fn token1_account(&mut self, token1_account: &'b solana_account_info::AccountInfo<'a>) -> &mut Self {
...
#[inline(always)]
  ...
  pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program_entrypoint::ProgramResult {
          let args = WithdrawInstructionArgs {
                                                              lp_token_amount: self.instruction.lp_token_amount.clone().expect("lp_token_amount is not set"),
                                                                  minimum_token0_amount: self.instruction.minimum_token0_amount.clone().expect("minimum_token0_amount is not set"),
                                                                  minimum_token1_amount: self.instruction.minimum_token1_amount.clone().expect("minimum_token1_amount is not set"),
                                    };
        ...
        __program: self.instruction.__program,
          ...
          owner_lp_token: self.instruction.owner_lp_token.expect("owner_lp_token is not set"),
                  
          token0_account: self.instruction.token0_account.expect("token0_account is not set"),
                  
          token1_account: self.instruction.token1_account.expect("token1_account is not set"),
